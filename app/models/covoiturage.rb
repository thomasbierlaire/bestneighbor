class Covoiturage < ApplicationRecord
  ...
  #    order by t.date")
  scope :not_taken, -> (utilisateur, code_postal) { joins(:user).where("user_id != ? AND takenby1 != ? AND takenby2 != ? AND takenby3 != ? and nbdispos > ? AND date >= ? AND users.code_postal = ?", utilisateur, utilisateur, utilisateur, utilisateur, 0, Time.now, code_postal).order(date: :asc) }
  scope :my_covoits, -> (utilisateur) { where("user_id = ? and date >= ?", utilisateur, Time.now).order(date: :asc) }
